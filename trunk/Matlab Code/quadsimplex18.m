function [alpha, b, iter, t, gcache, m] = quadsimplex18(P, T, C, ktype, lambda, giter, citer)
...
%         if (mod(cycles,citer) == 0 || min_g > -eps)
...
%         disp(['iteration = ', num2str(m_iter), ' min_g = ', num2str(min_g), ' idx = ', num2str(idx),' SVs = ', num2str(sum(m_svtype ~= 0))]);
...
%         if (idxfound == 0 && numMiss == 0)
%             plotSmallCurve(m_alpha, I, idxs, Q, m_C);
%         end
%         
...
%         h = zeros(size(m_T));
%         [h(idx_nb), gb] = solveSub(Q(idx_nb,idx_nb), m_T(idx_nb), +Q(idx_nb,I(idxs))*(-d(I(idxs))*m_C), 0);
...
%     fcache2 = zeros(size(m_fcache));
%     fcache2(idxzero) = -ones(length(idxzero),1) - m_beta * m_T(idxzero)'  - ...
%                         Q(idxzero, idxs) * m_alpha(idxs) - Q(idxzero, idxC) * m_alpha(idxC);
%     
%     fcache2(idxC) = ones(length(idxC),1) + m_beta * m_T(idxC)' + ...
%                         Q(idxC, idxs) * m_alpha(idxs) + Q(idxC, idxC) * m_alpha(idxC);
%     
%     if (sum(abs(fcache2 - m_fcache)) > 1e-6)
%         disp('Error');
%     end
%     
