function [alpha, b, iter, t, keval] = quadsimplex19(P, T, C, ktype, lambda, giter, citer)
	...
	elseif (ktype == 1),
        ...
        for i = 1:N,
             Q(i,:) = -exp(-lambda * sum((P(:,i)*ones(1,size(P,2)) - P).^2,1))*T(i).*T;
...
function plotSmallCurve(m_alpha, I, idxs, Q, C)

x = zeros(size(m_alpha));
x(I(idxs)) = 1;
r = [0:C/100:C];
alpha = m_alpha*ones(1,size(r,2)) + x*r;
for i = 1:101
    gs(i) = -0.5*alpha(:,i)'*Q*alpha(:,i) - sum(alpha(:,i));
end;
plot(gs)
pause(0.1)

function exhaustiveTestPairs()
    ...
    for i = 1:length(m_T)
        for j = i:length(m_T)
            ...
            idxs = [i; j];    
            alpha2 = m_alpha;
            alpha2(idxs) = m_C - alpha2(idxs);

            val = -sum(alpha2) - 0.5 * alpha2'*Q*alpha2 + sum(m_alpha) + 0.5*m_alpha'*Q*m_alpha;


            if (val < 0)
                disp(['idx = ',num2str(idxs(1)),', ',num2str(idxs(2))]);            
                return;
            end
...
function performGradProj()
    ...
    while (idxfound < Ns && numMiss < NMiss),
        ...
        % 
        % d'*(Q_rs*alpha_s + Q_rc * alpha_c - 1_r - 0.5*C*Q_rr*d) >= 0
        %
...
function FixComplementaryCondition()
    ...
    m_keval = m_keval + length(idx_nb)^2;
    ...
    while (max(abs(m_fcache(idx_nb))) > eps)
        ...
        if (~isempty(theta) && ( (theta < max(m_fcache(idx_nb)  ./ gamma)))),
           ...
           if (-h(idxr) > 0),
               m_svtype(idxr) = 2;
               m_keval = m_keval + size(Q,1)*length(idxr);
               
               upperfcache = upperfcache - Q(:,idxr)*m_C;
           else
               m_svtype(idxr) = 0;
           end;
...
function takeStep(idx)
    ...
    if (m_svtype(idx) == 0)
        ...
        m_keval = m_keval + 1 + length(idx_nb);
    ...
    if (bSlack)
        m_keval = m_keval + size(Q,1);
        upperfcache = upperfcache + Q(:,idx)*m_C;
    end
...
global m_keval;
   ...
   elseif (length(T) == 2),
       ...
       m_keval = m_keval + 2 + 2*(size(Q,1) - 1);
...
function updateNBCache()
    ...
    m_fcache(idxs) = -ones(length(idxs),1) - m_beta * m_T(idxs)'  - ...
                        Q(idxs, idxs) * m_alpha(idxs) + upperfcache(idxs); 
    
    m_keval = m_keval + length(idxs)^2;
...
global Q;
    ...
    m_fcache(idxzero) = -ones(length(idxzero),1) - m_beta * m_T(idxzero)'  - ...
                        Q(idxzero, idxs) * m_alpha(idxs) + upperfcache(idxzero); 
    
    m_fcache(idxC) = ones(length(idxC),1) + m_beta * m_T(idxC)' + ...
                        Q(idxC, idxs) * m_alpha(idxs) - upperfcache(idxC); 
                    
    
    m_keval = m_keval + length(idxzero)*length(idxs) + length(idxC)*length(idxs);