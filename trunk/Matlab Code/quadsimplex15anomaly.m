function [alpha, b, iter, t, gcache, m] = quadsimplex15(P, T, C, ktype, lambda)
...
%         disp(['iteration = ', num2str(m_iter), ' min_g = ', num2str(min_g), ' SVs = ', num2str(sum(m_alpha > 0))]);
...
function performGradProj()
    ...
    % The following is a shortcut method for computing -Qsc*alphac
    tmp = m_fcache(idx_nb)' + ones(size(idx_nb)) + m_T(idx_nb)'*m_beta + Q(idx_nb,idx_nb)*m_alpha(idx_nb);
    %disp(['KKT Violators = ',num2str(sum(y < 0))]);
    %disp(['Neg gradient ', num2str(sum((Q*m_alpha) < 0))]);
    %disp(['Neg gradient after project ', num2str(sum(((eye(length(m_T)) - m_T'*m_T/(m_T*m_T'))*Q*m_alpha) < 0))]);
    %disp(['Obj = ', num2str(-sum(m_alpha) - m_alpha'*Q*m_alpha)]);
    
    %obj = -sum(m_alpha) - m_alpha'*Q*m_alpha;
    
    while (Nfound < Ns),

        dir1 = ((m_svtype(I(idx1)) == 0) - .5) * 2;
        dir2 = ((m_svtype(I(idx2)) == 0) - .5) * 2;
        
        if (y(idx1) > eps), break; end;

        idxs = [idx1; idx2];
        N = 2;
        
        if ((m_T(I(idx1)) * dir1 + m_T(I(idx2)) * dir2 == 0.0) && ...
             (m_svtype(I(idx1)) ~= 1 && m_svtype(I(idx2)) ~= 1) && ...
             y(idx1) < -eps - .1 && y(idx2) < -eps - .1 )
            %val = ones(N,1)'*(+Q(I(idxs),:)*m_alpha + m_C * Q(I(idxs),I(idxs))*ones(N,1));
            val = ones(N,1)'*(y(idxs)'.*m_T(idxs)' + ones(N,1) + m_beta*m_T(idxs)' + Q(idxs,idxs)*ones(N,1));
%             d = -[dir1; dir2];
%             val = d'*(-Q(I(idxs),I(idxs))*m_alpha(I(idxs)) - Q(I(idxs),:)*m_alpha - ones(2,1) + m_C * Q(I(idxs),I(idxs))*d);
%             alpha = m_alpha;
%             alpha(I(idxs)) = m_C - alpha(I(idxs));
%             obj1 = -sum(m_alpha) - 0.5*m_alpha'*Q*m_alpha;
%             obj2 = -sum(alpha) - .5*alpha'*Q*alpha;
%             if (obj2 < obj1)
%                 val = 1;
%             else
%                 val = -1;
%             end
%             if ((val < 0 && obj2 < obj1) || (val > 0 && obj2 > obj1))
%                 disp('Whoops');
%             end
...
%         if (val > 0),
     ...
     %       disp(['Chose pair (', num2str(idx1),',',num2str(idx2),') and val = ', num2str(val)]);
            ...
            if (Nfound < Ns)
                ...
                while (idx1 <= length(m_T) && (check(idx1) == 1 || m_svtype(I(idx1)) == 1))
                    idx1 = idx1 + 1;
                end;
                
                if (idx1 >= length(m_T)) break; end;
    ...
    disp(['Obj after = ', num2str(-sum(m_alpha) - 0.5*m_alpha'*Q*m_alpha)]);
    ...
    disp(['Gradient Projection, found pairs = ', num2str(sum(check==1)), ' len(idx_nb) = ',num2str(lenb), ' -> ',num2str(length(idx_nb))]);
    disp(['Obj final = ', num2str(-sum(m_alpha) - 0.5*m_alpha'*Q*m_alpha)]);
