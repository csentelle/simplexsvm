...
%             val = d' * ( - Q(I(idxs),:) * m_alpha - ones(2,1) + 0.5 * m_C * Q(I(idxs),I(idxs)) * d);
            val = d' * ( -d.*(adj(I(idxs)) + m_fcache(I(idxs))') + m_beta * m_T(I(idxs))' + 0.5 * m_C * Q(I(idxs),I(idxs)) * d);
            
%             disp(['val = ', num2str(val), ' val2 = ', num2str(val2)]);
...
%     disp(['y * alpha = ', num2str(m_T * m_alpha)]);
