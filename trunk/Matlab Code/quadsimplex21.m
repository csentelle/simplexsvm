function [alpha, b, iter, t] = quadsimplex21(P, T, C, ktype, lambda, giter, citer)
%[alpha, b, iter, t] = quadsimplex21(P, T, C, ktype, lambda, giter, citer)
	global m_P;
	global m_T;
	global m_C;
	global m_fcache;
...
%     figh = figure;
    ...
    while (min_g < -eps || m_C < C)
        ...
        disp(['iteration = ', num2str(m_iter), ' min_g = ', num2str(min_g), ' idx = ', num2str(idx), ...
              ' SVs = ', num2str(sum(m_svtype ~= 0)), ' obj = ', ' obj = ', num2str(-sum(m_alpha) - 0.5*m_alpha'*Q*m_alpha)]);
...
%                 performGradProj(m_C*100/(cycles));
%                 plot_svm_results(P, T, m_beta, m_alpha', figh);
%                 pause
                performGradProj(0);
%                 plot_svm_results(P, T, m_beta, m_alpha', figh);                
%                 pause;
...
%     disp(['obj = ', num2str(-0.5*m_alpha'*Q*m_alpha - ones(1,length(m_T))*m_alpha)]);
    ...
    while (idxfound < Ns && numMiss < NMiss),
        ...
        val = d(I(idxs))' * ( -d(I(idxs)).*(adj(I(idxs)) + m_fcache(I(idxs))) + m_beta * m_T(I(idxs))' + ...
                     0.5 * m_C * Q(I(idxs),I(idxs)) * d(I(idxs))) + mu;
        
        objprev = (-sum(m_alpha)-0.5*m_alpha'*Q*m_alpha);
        ...
        if (val > 0)
            ...
            if ((-sum(m_alpha)-0.5*m_alpha'*Q*m_alpha > objprev ) ),
                disp('Whoops');
            end
    ...
    if (idxfound > 0),          
        ...
        objprev = (-sum(m_alpha)-0.5*m_alpha'*Q*m_alpha);
        FixComplementaryCondition();
        if (abs(-sum(m_alpha)-0.5*m_alpha'*Q*m_alpha - objprev) > 1e-3),
            disp('Whoops');
        end                   
        disp(['Num updates = ', num2str(idxfound)]);
        updateCache
%         disp(['obja = ', num2str(-0.5*m_alpha'*Q*m_alpha - ones(1,length(m_T))*m_alpha)]);

    end
%     disp(['obj3 = ', num2str(0.5*m_alpha'*Q*m_alpha - ones(1,length(m_T))*m_alpha)]);
