function [alpha, b, iter, t, gcache, m] = quadsimplex17(P, T, C, ktype, lambda, giter)
    ...
    while (min_g < -eps || m_C < C)
        ...
        if (mod(cycles, 10) == 0)
            exhaustiveTestPairs();
            %plot_svm_results(P,T,m_beta,m_alpha');
        end
...
%              ' SVs = ', num2str(sum(m_alpha > 0)), ' obj = ', num2str(-sum(m_alpha) - 0.5*m_alpha'*Q*m_alpha)]);
    ...
    disp(['Num SVs = ', num2str(sum(alpha > 1e-6))]);
    disp(['BSV = ', num2str(sum(alpha > m_C - 1e-6))]);
%     plot_svm_results(P,T,-m_beta,m_alpha');
...
function randTest()
    ...
    for k = 1:100,
        ...
        r = ceil(2.*rand(size(m_alpha)))-1;

        idx = find(m_svtype ~= 1);
        idx = idx(randperm(length(idx)));
        
        alpha2 = m_alpha;
        alpha2(idx(1:10)) = m_C - m_alpha(idx(1:10));


         valnew = -sum(alpha2) - 0.5 * alpha2'*Q*alpha2;
         valorg = -sum(m_alpha) - 0.5 * m_alpha'*Q*m_alpha;

         disp(['org val = ', num2str(valorg), ' new val = ', num2str(valnew)]);
